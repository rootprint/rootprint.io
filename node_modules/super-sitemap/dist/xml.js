 ...
 */
export function hasValidXmlStructure(xml) {
    const stack = [];
 ...
 */
function extractLocs(xml, entryTagName) {
    const locs = [];
    const entryRegex = new RegExp(`<${entryTagName}\\b[\\s\\S]*?<loc>([\\s\\S]*?)<\\/loc>[\\s\\S]*?<\\/${entryTagName}>`, 'g');
...
function decodeXmlText(value) {
    return value.replaceAll(/&(?:#(?<decimal>\d+)|#x(?<hex>[0-9a-fA-F]+)|(?<named>amp|apos|gt|lt|quot));/g, (entity, _decimal, _hex, named, _offset, _input, groups) => {
