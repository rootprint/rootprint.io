export interface Cell {
  /** "class:label" — class is `r` (brand) or `x` (muted); optional. */
  mark?: string;
  text: string;
  tone?: "win" | "lose" | "";
}
...
export interface Competitor {
  ...
  stayTitle: string;
  stayBullets: string[];
  /** Competitor column, aligned to the shared capability rows. */
  column: Cell[];
}

export const compareData: Record<string, Competitor> = {
  datadog: {
    slug: "datadog",
    navLabel: "Datadog",
    ...
    tldrTrade:
      "You give up the full APM/RUM/Synthetics suite. Bring your own dashboards and alerting.",
